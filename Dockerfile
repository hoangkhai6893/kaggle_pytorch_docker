...
RUN apt-get update  && apt install -y sudo nano
...
CMD ["sh","-c", "jupyter lab --notebook-dir=/home/${USER_NAME} --ip=0.0.0.0 --no-browser --allow-root --port=8888 --NotebookApp.token='' --NotebookApp.password='' --NotebookApp.allow_origin='*' --NotebookApp.base_url=${NB_PREFIX}"]
